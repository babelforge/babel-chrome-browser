...
- fallback `process-runtime` diagnostics while full native command execution is still being migrated;
...
- native routing from module-declared `babelchrome://<host>` URLs to native `process-web` module routes when applicable, with transitional LocalServiceHost fallback for remaining runtime paths;
