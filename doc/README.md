...
- a transitional LocalServiceHost/ExtensionHost runtime with the fresh public runtime contract for `static-web`, `process-web`, and `process-runtime`, plus renderers, view models, errors, Twig templates, source frontend assets, import maps, compiled public assets, dependencies supplied by installed modules, contextual menu item resolution, hook APIs, runtime context helpers, readiness checks, user-confirmed setup actions, legacy process lifecycle support, and remaining token-protected internal APIs;
- native routing from module-declared `babelchrome://<host>` URLs to native `process-web` and on-demand `process-runtime` module routes when applicable, with transitional LocalServiceHost fallback for remaining runtime paths;
