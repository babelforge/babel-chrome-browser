...
- native process runtime handling for `process-web` modules, including manifest validation, command definitions, port allocation, command interpolation, cwd resolution, environment preparation, readiness waiting, log capture, restart/stop/status actions, plus fallback `process-runtime` diagnostics;
