...
- shared viewer header controls for module viewers, including an `Open with` application selector backed by native BabelChrome internal endpoints and a generic message relay for compatible viewer tabs;
...
- native process runtime handling for `process-web` modules, including manifest validation, command definitions, port allocation, command interpolation, cwd resolution, environment preparation, readiness waiting, log capture, restart/stop/status actions, tokenized local HTTP route proxying, module `public/` asset serving, viewer source registration, viewer source status, viewer source assets, Open With, and viewer message relay acknowledgement;
- native on-demand `process-runtime` execution, including route-specific commands, stdin JSON payloads, stdout response decoding, timeout handling, environment preparation, and native diagnostics;
- a native module host with the fresh public runtime contract for `static-web`, `process-web`, and `process-runtime`, plus contextual menu item resolution, hook APIs, runtime context helpers, readiness checks, user-confirmed setup actions, and token-protected internal APIs;
- native routing from module-declared `babelchrome://<host>` URLs to native `process-web` and on-demand `process-runtime` module routes;
...
- the meta workspace `tools/dev2prod.sh` helper and runtime-aware `tools/ship-module.php` shipper for packaging Composer-based modules with their own locks and module-local `vendor/`, static web modules without Composer, and process-backed modules with their own executable runtime files;
