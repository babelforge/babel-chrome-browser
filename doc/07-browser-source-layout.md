...
- `Runtime`: native process runtime definitions, process-web start/stop/status, port allocation, command interpolation, cwd resolution, readiness waiting, log capture, tokenized process-web HTTP proxying, module public asset serving, on-demand process-runtime command execution, and runtime diagnostics.
