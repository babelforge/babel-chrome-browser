...
BrowserWindowController
    ...
    |__ Services and stores
            ...
            |__ ModuleInternalPageHTMLBuilder
            ...
            |__ ViewerSourceActionHandler
            |__ ViewerNavigationURLResolver
            ...
            |__ DeveloperToolsTargetResolver
            |__ BrowserThemeApplier
...
| --- | --- |
| `BrowserWindowController+AddressAndSuggestions.inc.mm` | Selected tab address display, address field editing, CEF browser title/URL/loading/favicons/status updates, and omnibox suggestions. Address input normalization and final navigation request resolution are delegated to `BabelAddressNavigationRequestResolver`; displayed-vs-actual address navigation resolution is delegated to `BabelAddressFieldNavigationResolver`; address badge/text-field frame calculation is delegated to `BabelAddressFieldLayoutCalculator`; link-hover status-bar presentation is delegated to `BabelLinkStatusBarController`; window title, compact tab title, and badge color formatting are delegated to `BabelBrowserPresentationFormatter`; address display URL and viewer badge resolution are delegated to `BabelAddressBarDisplayResolver`; browser link copy actions are delegated to `BabelBrowserPasteboardWriter`; title/favicon callback application is delegated to `BabelBrowserTabMetadataUpdater`; favicon persistence is delegated to `BabelFaviconStore`; local suggestion row collection and suggestion favicon lookup are delegated to `BabelOmniboxSuggestionContextBuilder`; local suggestion matching is delegated to `BabelOmniboxLocalSuggestionBuilder`; Google Suggest HTTP/cache/search URL work is delegated to `BabelGoogleSuggestClient`; Google Suggest debounce and stale-response rejection are delegated to `BabelGoogleSuggestionScheduler`; suggestion panel state/rendering is delegated to `BabelOmniboxSuggestionsController`. |
...
### `BabelTabDragHoverScheduler`

`BabelTabDragHoverScheduler` owns delayed group selection while dragging a tab over the sidebar:

- tracks the group currently pending hover selection;
- cancels pending hover selection when the drag leaves a valid group;
- waits for the hover delay before selecting a group;
- rejects stale delayed selections through generation-based cancellation.

The controller may still provide geometric validation and perform the final group selection. It must not own hover-delay dispatch timers or generation counters inline.
...
### `BabelModuleInternalPageHTMLBuilder`

`BabelModuleInternalPageHTMLBuilder` owns complete HTML document assembly for module internal pages:

- collects the installed module snapshot for the Modules page;
- collects release-manifest and installed-module data for the Module Updates page;
- builds the Module Details page from module snapshots;
- wraps module page bodies in the shared internal page shell.

The controller may still decide when a module internal page should open. It must not collect module page snapshots or assemble module page HTML inline.
